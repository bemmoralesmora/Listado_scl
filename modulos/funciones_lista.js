...
export async function cargarAlumnosPorGradoYFecha(contenedor, nombreGrado, fecha) {
    try {
    ...
    try {
        ...
        alumnosItems.forEach(item => {
            ...
            asistencias.push({
                ...
                comentario: comentarioInput.value || null
            });
        });
...
export async function obtenerAlumnosBasicosPorGrado(nombreGrado) {
    try {
        ...
        const gradoData = await gradoResponse.json();
        const alumnosResponse = await fetch(`http://localhost:3000/grados/${gradoData.id_grado}/alumnos`);
...
export async function cargarAsistenciaGuardada(grado, fecha) {
    try {
        const response = await fetch(`http://localhost:3000/asistencia/grado/${encodeURIComponent(grado)}?fecha=${fecha}`);
